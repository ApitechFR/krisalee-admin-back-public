...
function oidc_keycloak_map_user_role( $user, $user_claim ) {
	...
	error_log(json_encode($user_claim));
...
/**
 * Set WordPress email based on Keycloak username after authentication.
 *
 * @param WP_User      $user       The authenticated user's WP_User object.
 * @param array<mixed> $user_claim The IDP provided Identity Token user claim array.
 *
 * @return void
 */
function oidc_keycloak_map_user_username( $user, $user_claim ) {
	// Get keycloak username
	$username = $user_claim['preferred_username'];
	// Check if the Keycloak username is available in the user claim
    if (!empty($username)) {
		// Prepare the internal email by adding the domain name to the username
		$internal_email = explode('@', $username)[0].'@krisalee.dev.joona.fr';
        // Set the WordPress user email to the user internal email
        $user->user_email = $internal_email;
...
function oidc_keycloak_map_internal_email( $user, $user_claim ) {
	// Check if the internal_email
	if (!empty($user_claim['internal_email'])) {
			$user->user_email = $user_claim['internal_email'][0];
			// Update the user record
			wp_update_user($user);
			// Set the updated user as the current user
			wp_set_current_user($user->ID);
...
add_action('openid-connect-generic-update-user-using-current-claim', 'oidc_keycloak_map_internal_email', 10, 2);
add_action('openid-connect-generic-user-create', 'oidc_keycloak_map_internal_email', 10, 2);

function custom_head_code() {
	// get the current wordpress logged in user
	$current_user = wp_get_current_user();
    // show the user internal email
    echo '<p style="width: fit-content; background-color: #e4002b; color: white; margin-left: auto; margin-right: auto; padding: 5px 15px; border-radius: 5px; display: flex; flex-direction: column; align-items: center;">';
	echo '<span>Email: </span>';
	//echo '<br>';
	echo '<span style="font-weight: bold;">'.$current_user->data->user_email.'</span>';
